...
// Ported from ESP32 version — no hardware dependencies
...
enum ArpPattern {
  ...
  ARP_FOURTHS_UP, ARP_FOURTHS_DOWN, ARP_OCTAVES, ARP_REPEAT_2X,
  ARP_REPEAT_3X, ARP_PENDULUM, ARP_PING_PONG_2, ARP_PING_PONG_3,
  ...
  ARP_EUCLIDEAN_5_8, ARP_EUCLIDEAN_7_12, ARP_POLYRHYTHM_3_4, ARP_POLYRHYTHM_5_4,
  ARP_CHORD_INVERSION, ARP_ALTERNATING, ARP_PYRAMID, ARP_CASCADE,
  ARP_SINE_WAVE, ARP_TRIANGLE_WAVE, ARP_SQUARE_WAVE, ARP_SAW_WAVE,
  ARP_PROBABILISTIC, ARP_FRACTAL, ARP_CHAOS, ARP_RHYTHMIC_1,
  ARP_RHYTHMIC_2, ARP_CLASSIC_808
};
...
static const char* arpPatternNames[NUM_ARP_PATTERNS] = {
  ...
  "Euclid5/8","Euclid7/12","Poly 3/4","Poly 5/4",
...
private:
  ...
  void generateEuclideanRhythm(int steps, int pulses) {
    ...
    for (int i = 0; i < steps; i++) {
      bucket += pulses;
      if (bucket >= steps) { bucket -= steps; if (i < 16) euclideanRhythm[i] = true; }
    }
  }
  ...
  int getNextNoteIndex() {
    ...
    switch (pattern) {
      case ARP_UP:
        idx = currentStep % noteCount; break;
      case ARP_DOWN:
        idx = noteCount - 1 - (currentStep % noteCount); break;
      case ARP_UP_DOWN: {
        int cycle = (noteCount - 1) * 2;
        int pos = currentStep % max(cycle, 1);
      ...
      case ARP_RANDOM:
        idx = random(noteCount); break;
      case ARP_ORDER_PLAYED:
        idx = currentStep % noteCount; break;
      case ARP_CONVERGE: {
        int half = max(noteCount / 2, 1);
        idx = ((currentStep / half) % 2 == 0) ? (currentStep % half) : (noteCount - 1 - (currentStep % half));
        break;
      }
      case ARP_DIVERGE: {
        int half = noteCount / 2;
        int pos = currentStep % max(noteCount, 1);
        idx = (pos % 2 == 0) ? (half + pos / 2) : (half - 1 - pos / 2);
        break;
      }
      case ARP_THIRDS_UP:
        idx = ((currentStep % noteCount) + (currentStep / noteCount) * 2) % noteCount; break;
      case ARP_FOURTHS_UP:
        idx = ((currentStep % noteCount) + (currentStep / noteCount) * 3) % noteCount; break;
      case ARP_OCTAVES:
        idx = currentStep % noteCount;
        currentOctave = (currentStep / noteCount) % maxOctaves;
        break;
      case ARP_REPEAT_2X:
        idx = (currentStep / 2) % noteCount; break;
      case ARP_REPEAT_3X:
        idx = (currentStep / 3) % noteCount; break;
      case ARP_PENDULUM:
        idx = (currentStep % 2 == 0) ? 0 : min((currentStep / 2) + 1, noteCount - 1);
        break;
      case ARP_ZIGZAG: {
        int base = currentStep / 2;
        idx = (currentStep % 2 == 0) ? base : (base + noteCount / 2);
        idx = idx % max(noteCount, 1);
        break;
      }
      case ARP_BOUNCE:
        idx = abs((currentStep % max(noteCount * 2, 1)) - noteCount);
        break;
      case ARP_STUTTER:
        idx = (currentStep / 2) % noteCount; break;
      case ARP_ALTERNATING:
        idx = (noteCount < 2) ? 0 : ((currentStep % 2 == 0) ? 0 : (noteCount - 1));
        break;
      case ARP_PYRAMID: {
        int level = (currentStep / max(noteCount, 1)) % (noteCount + 1);
        if (level == 0) level = 1;
        idx = currentStep % max(level, 1);
        break;
      }
      case ARP_SINE_WAVE: {
        float ph = (currentStep * 6.28318f) / max(noteCount, 1);
      ...
      case ARP_PROBABILISTIC:
        ...
        break;
      default:
        idx = currentStep % max(noteCount, 1); break;
    }
...
public:
  Arpeggiator() {
    noteCount = 0; currentStep = 0; currentOctave = 0; maxOctaves = 1;
    lastStepTime = 0; bpm = 120.0f; pattern = ARP_UP;
    isPlaying = false; gateLength = 80;
    repeatCounter = 0; direction = 1; patternPhase = 0; euclideanPos = 0;
    generateEuclideanRhythm(8, 5);
  }

  void noteOn(int note) {
    bool found = false;
    for (int i = 0; i < noteCount; i++) if (heldNotes[i] == note) { found = true; break; }
    if (!found && noteCount < MAX_ARP_NOTES) {
      heldNotes[noteCount++] = note;
      for (int i = 0; i < noteCount - 1; i++)
        for (int j = i + 1; j < noteCount; j++)
          if (heldNotes[i] > heldNotes[j]) { int t = heldNotes[i]; heldNotes[i] = heldNotes[j]; heldNotes[j] = t; }
    }
    if (!isPlaying && noteCount > 0) { isPlaying = true; currentStep = 0; lastStepTime = millis(); }
  }
  ...
  void allNotesOff() { noteCount = 0; isPlaying = false; currentStep = 0; }
  ...
  void setPattern(ArpPattern p) {
    ...
    if (pattern == ARP_EUCLIDEAN_7_12) generateEuclideanRhythm(12, 7);
  }

  void setBPM(float b)         { bpm = constrain(b, 30.0f, 300.0f); }
  void setOctaveRange(int o)   { maxOctaves = constrain(o, 1, 4); }
  void setGateLength(int g)    { gateLength = constrain(g, 10, 100); }
